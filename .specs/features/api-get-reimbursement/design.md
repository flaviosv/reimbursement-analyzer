...
| ------- | --------------------- | ------ | ---------- |
| `repository.py` is the only module in the workspace issuing SQL against the `reimbursement`/`human_review` tables — an invariant this design doc infers from the codebase, not a quoted statement (the file's actual docstring is `"""Every SQL statement against the \`reimbursement\` table."""`; pool lifecycle itself lives in `shared/db.py` per AD-029) | `src/shared/src/shared/reimbursement/repository.py:1` | A future author could be tempted to query directly from `route.py` for a one-off need, breaking the single-owner invariant | Keep all SQL in `repository.py`; `route.py` only calls the use case, never `fetch_reimbursement_page()` directly |
