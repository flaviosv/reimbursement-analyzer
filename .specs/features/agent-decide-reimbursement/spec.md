...
| Decision-stage error-handling mechanism | ~~Not selected in this spec... deferred to a later session — see **R-011**~~ **Resolved 2026-08-10** by `agent-decision-error-escalation`: immediate escalation to `human-review`, not retry-then-escalate or a cause-differentiated policy — see **AD-039** in `.specs/STATE.md` | User: "Add this note to RISKS.md ... we must avoid spent unnecessary cost, so another moment is gonna be evaluated" | y |
