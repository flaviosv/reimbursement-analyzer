...
| PII minimization in prompts | Every LLM prompt in this feature avoids including PII (e.g. `submitted_by`) beyond what the step operationally needs | `SCOPE.md:59` marks this "(?)" (uncertain) in the original doc, but it's a low-cost, defensible default for a financial-domain LLM integration | n |
