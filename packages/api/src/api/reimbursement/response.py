...
class ReimbursementItem(BaseModel):
    ...
    def from_record(cls, record: asyncpg.Record) -> Self:
        """Build one item from a fetch_reimbursement_page()/
        fetch_reimbursement_by_uuid() row — the hr_*-prefixed LATERAL
        columns are NULL together when no human_review row exists, non-NULL
