...
- **Wire up LLM tracing.** LangFuse is provisioned in `docker-compose.yml` but not yet wired into any code (`docs/codebase/INTEGRATIONS.md`). Any new agent/LLM code path must emit a trace to it — this is required before that code path ships, not deferred as follow-up.
