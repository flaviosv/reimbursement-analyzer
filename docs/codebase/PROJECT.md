...
- Classify every decision as auto-approved, human-review, or auto-rejected — **implemented**: `reimbursement`'s LangGraph decision graph applies the deterministic reject/auto-approve/mandatory-human-review thresholds first, then an LLM-as-judge guardrail for the ambiguous zone in between.
...
- **`api`** — the full reimbursement HTTP surface is implemented and tested: `POST /api/v1/reimbursement` (intake — request validation, a 1 MiB body ceiling, publish to Kafka), `GET /api/v1/reimbursement` (list, with status/limit/offset filtering), `GET /api/v1/reimbursement/{uuid}` (single-record detail lookup), and `PUT /api/v1/reimbursement/{uuid}` (records a human reviewer's approve/reject decision). **PUT is a human-driven decision-recording endpoint, not the automated decision layer** — a reviewer calls it after making up their own mind on a row already at `human-review`/`auto-rejected`/`human-rejected`; it does not itself evaluate or classify anything. The automated decision logic lives in `reimbursement`'s decision graph (below).
...
- **`reimbursement`** (the package formerly named `agent`, renamed and flattened to match `publisher`/`api`'s layout) — both its consume/resolve layer and its decision layer are implemented and tested. Consume/resolve: consumes `Reimbursement`, resolves the row by `uuid`, tolerates a stale or ghost message, requeues transient failures, and escalates past its own retry ceiling to `human-review` (reusing the same escalation action `publisher` uses); a decision-graph failure escalates the same way, immediately, regardless of retry count. Decision: a LangGraph `StateGraph` (the nested `agent/` subpackage), invoked by `validation.py`'s resolve path for every resolved row — `extract_fields`/`validate` gate on completeness, `apply_policies` applies the deterministic rules (reject receipts older than 90 days, auto-approve ≤R$200, mandatory human-review >R$2000), and `analysis` runs an LLM-as-judge guardrail for the ambiguous R$200–2000 zone before `apply_agent_decision` persists the outcome; every LLM call is traced via LangFuse; both LLM nodes are backed by Groq, independently configured (AD-032, `docs/codebase/STACK.md`).
- Database schema exists for `reimbursement` and `human_review`. `reimbursement` rows are written by `publisher` (`pending` on success, `human-review` past its retry ceiling), by `reimbursement`'s own decision graph (`auto-approved`/`auto-rejected`/`human-review`, plus `human-review` past its resolve-path retry ceiling or immediately on a decision-graph failure), and by `api` (`human-approved`/`human-rejected` via `PUT`). `human_review` is written by both `reimbursement`'s escalation path and `api`'s `PUT` endpoint (via `review_reimbursement`), each appending one row per decision in the same transaction as the status update.
