...
- **`reimbursement`** (the package formerly named `agent`, renamed and flattened to match `publisher`/`api`'s layout) — both its consume/resolve layer and its decision layer are implemented and tested. Consume/resolve: consumes `Reimbursement`, resolves the row by `uuid`, tolerates a stale or ghost message, requeues transient failures, and escalates past its own retry ceiling to `human-review` (reusing the same escalation action `publisher` uses). Decision: a LangGraph `StateGraph` (the nested `agent/` subpackage), invoked by `validation.py`'s resolve path for every resolved row — `extract_fields`/`validate` gate on completeness, `apply_policies` applies the deterministic rules (reject receipts older than 90 days, auto-approve ≤R$200, mandatory human-review >R$2000), and `analysis` runs an LLM-as-judge guardrail for the ambiguous R$200–2000 zone before `apply_agent_decision` persists the outcome; every LLM call is traced via LangFuse. **Currently broken in the uncommitted working tree**: a prompt-authoring refactor in progress drops an export both `agent.py` and the entire `reimbursement` test suite still import — see `docs/codebase/CONCERNS.md`'s Known Bugs.
- Database schema exists for `reimbursement` and `human_review`. `reimbursement` rows are written by `publisher` (`pending` on success, `human-review` past its retry ceiling), by `reimbursement`'s own decision graph (`auto-approved`/`auto-rejected`/`human-review`, plus `human-review` past its resolve-path retry ceiling), and by `api` (`human-approved`/`human-rejected` via `PUT`). `human_review` is written by both `reimbursement`'s escalation path and `api`'s `PUT` endpoint (via `review_reimbursement`), each appending one row per decision in the same transaction as the status update.
