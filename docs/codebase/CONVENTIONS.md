...
- **README:** `README.md` documents install/run/test/migrate at a high level; one line still states the migrations directory one path segment too deep (`packages/api/src/api/migrations/` vs. the real `packages/api/src/migrations/`) — see `CONCERNS.md`.
