...
A second, `api`-scoped pattern exists alongside this one: `shared.logging.log_event(logger, level, event, **fields)` (`packages/shared/src/shared/logging.py`) builds one JSON event line (`{"event": ..., **fields}`), mirroring `failure_log.write`'s `default=str` + defensive try/except shape. Adopted only by `api`'s 4 route files for their own new call sites — not a replacement for the three-tier pattern above, which stays exactly as documented. Always imported as `from shared.logging import log_event`, never `from shared import logging` — the module's own docstring states this convention explicitly, to avoid reading like the stdlib module at the call site.
