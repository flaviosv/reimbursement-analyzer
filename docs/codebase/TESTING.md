...
- **Decision graph (`reimbursement/agent/`):** one test file per node (`test_extract_fields.py`, `test_validate.py`, `test_apply_policies.py`, `test_analysis.py`, `test_apply_agent_decision.py`) plus `test_agent.py`, which wires the real graph shape (`agent._wire`) against fakes at every LLM/DB boundary to cover routing end to end; `test_langfuse.py` covers the tracing fallback. `conftest.py` and 3 of the per-node files (`test_validate.py`, `test_apply_policies.py`, `test_apply_agent_decision.py`) collect and pass. **4 files currently uncollectable** — `test_agent.py`, `test_analysis.py`, `test_extract_fields.py`, `test_integration.py` still import a name a prompt refactor removed; owned by the in-flight sibling feature `agent-model-config`, see `docs/codebase/CONCERNS.md`'s Known Bugs.
