...
- Location: `src/publisher/src/processing.py` (`_insert_and_publish` → `shared.reimbursement.use_cases.publish_pending`), `src/reimbursement/src/consumer.py` (consume lifecycle), `src/reimbursement/src/validation.py` (resolve, requeue)
...
- Location: `src/api/src/migrations/*.sql`, `src/api/src/migrate.py` (schema); `src/shared/src/shared/db.py` (pool construction — relocated here from `repository.py`, AD-029); `src/shared/src/shared/reimbursement/repository.py` (SQL statements); `src/reimbursement/src/validation.py` (reimbursement service's call sites); `src/api/src/reimbursement/list/route.py`, `src/api/src/reimbursement/update/route.py`, `src/shared/src/shared/reimbursement/use_cases/{list_reimbursements,review_reimbursement}.py` (api's call sites); `src/api/src/main.py` (pool constructed in `lifespan`), `src/api/src/dependencies.py` (`get_pool`)
