...
| `shared.tracing.{init_tracer,traced_message_span,stamp_span}` | Shared OTel wiring — tracer provider lifecycle, Kafka header inject/extract, the two cross-hop span attributes every service's composition root and routes call into |
...
LangFuse (previously provisioned in this repo's `docker-compose.yml`, removed entirely this branch — AD-040; local LangFuse provisioning is now the sibling `local-env` project's concern) is wired into `reimbursement`'s LangGraph decision graph (`agent/agent.py`'s `_langfuse_handlers()`): a memoized `langfuse.langchain.CallbackHandler` is passed to every `graph.ainvoke(...)` call, alongside `metadata={"langfuse_session_id": str(reimbursement.uuid), "correlation_id": ...}` (the latter omitted when `None`) — every trace for one reimbursement (including retries) groups under one LangFuse Session, filterable by uuid, and cross-referenceable to the same request's logs by `correlation_id`. If the package is missing or the handler can't be constructed, a durable `failure_log` record (`reimbursement.langfuse_fallback`) is written instead — no invocation runs trace-less and unlogged. See `docs/codebase/INTEGRATIONS.md` for the full data-flow description.

**Distributed tracing (OpenTelemetry, all three services):** `shared.tracing` (`packages/shared/src/shared/tracing.py`) is the single place OTel SDK setup, the OTLP endpoint, and the Kafka-header wire format for trace context all live. It exposes `init_tracer(service_name, otlp_endpoint) -> TracerProvider` (called once per service at its own composition-root startup — `api/main.py`'s module scope + `lifespan`, `publisher`/`reimbursement`'s `_serve()`), `shutdown_tracer`/`shutdown_tracer_async` (clean shutdown at each service's existing shutdown hook), `stamp_span(span, *, uuid=None, correlation_id=None)` (centralizes the two cross-hop attribute names — `reimbursement.uuid` and `correlation_id` — so every call site sets exactly the same keys), `inject_headers()`/`extract_context()` (serialize/deserialize the current trace context to/from the Kafka message-header wire shape `confluent_kafka` expects), and `traced_message_span(tracer, message)` (a context manager wrapping one consumed Kafka message's handling in a span that is a child of whatever trace context its headers carried, or a new root if none — used identically by `publisher`'s and `reimbursement`'s consume loops).

`api`'s HTTP layer is auto-instrumented via `FastAPIInstrumentor.instrument_app(app)` at startup (`main.py`) — no per-route span-creation code needed there; the four routes (`create`/`get`/`list`/`update`) additionally call `stamp_span()` explicitly to attach `correlation_id` (and `reimbursement.uuid` where already resolved) onto the auto-created request span. Trace context propagates across Kafka the same way `correlation_id` does: `shared.producer.publish()` injects the current trace context into the outbound message's Kafka headers via `inject_headers()`; `publisher`'s and `reimbursement`'s consume loops (`consumer.py`'s `run()`) wrap each message in `traced_message_span()`, which calls `extract_context()` on the inbound headers first — so one trace spans the full `api` → `publisher` → `reimbursement` hop chain, the same way `correlation_id` does for logs.

Every span stamps `correlation_id` wherever it's in scope (every hop, from the first) and `reimbursement.uuid` wherever a row's identity is already resolved (kept independently, per `AD-041` in `.specs/STATE.md`, since a direct DB/log lookup by uuid is a distinct query shape `correlation_id` doesn't substitute for) — both via the same `stamp_span()` helper, never a bespoke `set_attribute()` call at each site. If exporting a span fails or the OTLP endpoint is unreachable, this is OTel SDK's own concern (batched, best-effort export) — no application-level fallback exists or is needed here, unlike LangFuse's explicit `failure_log` fallback (LLM traces are a harder traceability requirement; infrastructure spans are not).
