...
| Metrics | Prometheus instrumentation — HTTP RED metrics + a scrape-time status Gauge (`api`, on the main HTTP port), Kafka/decision-graph counters and histograms (`publisher`/`reimbursement`, each on its own standalone `METRICS_PORT`) | `packages/shared/src/shared/metrics.py`, `packages/api/src/api/metrics.py` (+ `middleware.py`'s `MetricsMiddleware`), `packages/publisher/src/publisher/metrics.py`, `packages/reimbursement/src/reimbursement/metrics.py` |
...
- Inside `api`, vertical slices (`reimbursement/create/`) own their route, validation, and publish logic; app-root modules (`main.py`, `dependencies.py`, `errors.py`) hold only what is genuinely slice-independent — e.g. `dependencies.py`'s `get_producer` exists as a standalone module specifically to avoid a `main.py` ↔ `route.py` import cycle. **Exception:** the metrics module (`api/metrics.py`) calls `shared.reimbursement.repository.count_by_status` directly at scrape time (not wrapped by a use-case layer) because `/metrics` is an operational/monitoring endpoint, not a user-facing request — it is not subject to the usual request/response validation gates and has different latency/availability tradeoffs (see CONCERNS.md P1).
...
5. Once a row resolves, `validation.py` invokes `agent.decide()` (the decision graph below) before the consumer offset commits — never mid-message. A decision-graph failure (an LLM call failure, malformed structured output, or a genuine persist failure inside one of the graph's write nodes) is caught here and escalated to `human-review` immediately, reusing the same `escalate_existing` action as step 2's retry-ceiling escalation — never retried, and never left untouched; a decision computed in-memory but never durably persisted is discarded in favor of the escalation.
...
| `reimbursement.validation.handle_message` | The service's entire resolve decision tree — resolve by uuid, staleness guard, ghost tolerance, retry/requeue, retry-ceiling escalation, immediate decision-stage-failure escalation — never raises |
...
| `api.middleware.CorrelationIdMiddleware` | Raw ASGI middleware — mints or echoes the request's `correlation_id`, sets it on `shared.logging`'s `ContextVar` for the request's duration, echoes it back as the `X-Request-ID` response header |
| `shared.logging.configure_logging` | Every service's single logging entrypoint — attaches one ECS-JSON-formatting handler carrying `CorrelationIdFilter` to the root logger, resolves `LOG_LEVEL` |
| `shared.metrics.start_metrics_server` | Starts `publisher`/`reimbursement`'s standalone `/metrics` HTTP server on `METRICS_PORT`, off the process's default `prometheus_client` registry; also owns the one metric two services write to (`reimbursement_status_transitions_total`) |
| `api.middleware.MetricsMiddleware` | Raw ASGI middleware recording `api_http_requests_total`/`api_http_request_duration_seconds` for every request, including error responses |
| `api.metrics.refresh_status_gauge` | Scrape-time-only DB query refreshing `reimbursement_status_count`; failure logs a warning and returns rather than failing the whole `/metrics` response |
| `reimbursement.agent.nodes.apply_policies.PolicyRule` | 3-value `str, Enum` labeling `reimbursement_policy_rule_triggered_total` (`stale-receipt-reject`, `low-value-auto-approve`, `high-value-human-review`) — a value in the ambiguous zone fires none of them |

## Data Model

- **`reimbursement`** — one row per submitted request: `uuid` (PK, `uuidv7()`), `request_id`, `submitted_by`, `submitted_at`, `original_payload` (JSONB), `status` (`pending` / `auto-approved` / `auto-rejected` / `human-review` / `human-approved` / `human-rejected`), `receipts_value`, `receipts_date`, `currency`, `decision_reason`, `human_review_notes`, `created_at`, `updated_at`. Unique on `(request_id, lower(submitted_by))`; an index on `created_at` supports `GET`'s default no-filter ordering. **Actively read and written by all three services**: `publisher` inserts a `pending` row per item (or a `human-review` row past its own retry ceiling); `reimbursement` reads it by `uuid` to resolve into a `shared.models.Reimbursement`, then its decision graph writes `status`/`decision_reason`/`receipts_value`/`receipts_date`/`currency` (`auto-approved`/`auto-rejected`/`human-review`), or `status='human-review'` alone via `escalate_existing` — past its own resolve-retry ceiling (the same action `publisher` uses) or, immediately, on a decision-graph failure; `api` reads a filtered/paginated page (or a single row by `uuid`) for `GET`, and atomically transitions a row to `human-approved`/`human-rejected` (backfilling `receipts_value`/`receipts_date`/`currency` on approve) for `PUT`.
...
`reimbursement.validation` uses the identical shape for its own decision tree: every branch of `handle_message` returns a `MessageOutcome` member (`RESOLVED | STALE | GHOST | REQUEUED | ESCALATED | LOGGED | INVALID`) and never raises — one bad message can never stop the consume loop behind it. A decision-graph failure escalates to `human-review` immediately rather than entering the retry/requeue path, carrying the full attempt history (any prior resolve-stage retries plus the new failure) through the existing `AttemptError`/`render_history` machinery.

## Observability

Three-tier logging, all via stdlib `logging` (no OpenTelemetry; Prometheus metrics are now covered separately below):
...
**Structured JSON logging + correlation-id propagation** (`shared.logging`, all three services): every service calls `configure_logging()` exactly once at startup (`api/main.py`, `api/migrate.py`, `publisher/consumer.py`, `reimbursement/consumer.py`), replacing the former per-service `logging.basicConfig(...)` calls. It attaches one root-logger handler formatted by `ecs_logging.StdlibFormatter()`, so every log line across `api`/`publisher`/`reimbursement` is now a single JSON object with ECS field names, not just `api`'s own event-logging call sites. Verbosity is controlled by the `LOG_LEVEL` env var (`debug`/`info`/`warning`/`error`/`critical`, case-insensitive, default `debug`; an invalid value falls back to `debug` with one `warning` log line, never a crash). `shared.logging.log_event(logger, level, event, **fields)` emits `event` and `**fields` as real structured JSON keys via `extra={}` (not a hand-rolled `json.dumps()` message string), coercing `UUID`-typed field values to strings; used by `api`'s 4 routes (`request_id` on `POST`, `uuid` on `GET`/`GET`-by-uuid/`PUT`) and `api/errors.py`'s catch-all handler.

A `correlation_id` (one per HTTP request) threads through the entire pipeline via one module-level `contextvars.ContextVar` plus a `CorrelationIdFilter` (a `logging.Filter`) that injects the current context's id into every `LogRecord` automatically — no per-call-site `extra={}` needed. Chain, end to end: the `X-Request-ID` request header is echoed back if present (non-empty, ≤128 chars, no CR/LF) or a fresh `uuid.uuid7()` is minted, by `api.middleware.CorrelationIdMiddleware` — a raw ASGI middleware, not `BaseHTTPMiddleware`/`@app.middleware("http")`, chosen specifically to avoid Starlette's documented `ContextVar`-propagation edge cases around its task-group/streaming-response handling. The middleware sets the `ContextVar` for the request's duration (reset in a `finally`) and echoes the same value back as the `X-Request-ID` response header. `api`'s producer (`reimbursement/create/producer.py`) reads it via `get_correlation_id()` and splices it onto `RequestEnvelope.correlation_id`. `publisher.processing.handle_message` sets its own process's `ContextVar` from `envelope.correlation_id` for the duration of that one message (reset in a `finally`), carrying it forward onto the `ReimbursementEnvelope` it publishes and onto any requeued `RequestEnvelope`. `reimbursement.validation.handle_message` does the same from the `ReimbursementEnvelope`, additionally passing it into `agent.decide()`'s LangFuse `metadata` dict as `correlation_id`. A `None` correlation_id at any hop (a pre-feature message, or no incoming header) is normal — the field/key is simply omitted, never emitted as `null`. Together, a reimbursement's full lifecycle (api write → publisher → agent nodes → LangFuse trace) is grep- and LangFuse-session-filter-reconstructable end to end by one id, without re-running anything.

LangFuse (provisioned in `docker-compose.yml`) is wired into `reimbursement`'s LangGraph decision graph (`agent/agent.py`'s `_langfuse_handlers()`): a memoized `langfuse.langchain.CallbackHandler` is passed to every `graph.ainvoke(...)` call, alongside `metadata={"langfuse_session_id": str(reimbursement.uuid), "correlation_id": ...}` (the latter omitted when `None`) — every trace for one reimbursement (including retries) groups under one LangFuse Session, filterable by uuid, and cross-referenceable to the same request's logs by `correlation_id`. If the package is missing or the handler can't be constructed, a durable `failure_log` record (`reimbursement.langfuse_fallback`) is written instead — no invocation runs trace-less and unlogged. See `docs/codebase/INTEGRATIONS.md` for the full data-flow description.

**Prometheus metrics** — 16 metrics (1 Gauge, 5 Histograms, 10 Counters), full catalog in `docs/METRICS.md`. Each of `api`/`publisher`/`reimbursement` exposes its own `/metrics` off that process's own default `prometheus_client` registry, with no shared registry or federation layer between them — Prometheus scrapes all 3 targets independently, since an in-process counter constructed in one process is invisible to another's `/metrics` endpoint. `api`'s `/metrics` rides its existing FastAPI HTTP port; `publisher`/`reimbursement` each bind a standalone `METRICS_PORT` (default `9101`/`9102`) via `shared.metrics.start_metrics_server`, called once at consumer startup, same fail-fast-on-bind-failure convention as `check_startup_config()`. `api.middleware.MetricsMiddleware` records `api_http_requests_total`/`api_http_request_duration_seconds` for every request (including error responses); `reimbursement_status_count` (a Gauge) is refreshed from a live DB query at scrape time rather than incrementally maintained, and a failed refresh logs a warning and still serves the other 15 metrics rather than failing the whole endpoint. `reimbursement_status_transitions_total` is the one metric two services write to, living in `shared.metrics` for that reason. None of the 3 `/metrics` endpoints has authentication (see `docs/codebase/CONCERNS.md`). Every `Counter(...)` is constructed without the `_total` suffix — `prometheus_client` appends it at exposition time.
